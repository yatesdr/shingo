...
| Order Type | `order_type` | string | Yes | One of: `"retrieve"`, `"retrieve_empty"`, `"move"`, `"complex"`. |
...
| Source Node | `source_node` | string | Conditional | Where to pick up. Required for `move`. Optional for `retrieve` / `retrieve_empty`, where it names a supermarket group to pull from; blank means Core's global FIFO search. |
...
- `move` -- Move material between two specified locations. Both `source_node` and `delivery_node` are required.
- `retrieve_empty` -- Fetch an EMPTY carrier rather than a loaded one. A first-class type, not a flag.
- `complex` -- A multi-step order (pickup / dropoff / wait), used for swaps and staged moves. Carries `steps` rather than a single source and destination.

There is no `store` order type. It appeared in this table and in the waybill example below for a long time without ever existing in `protocol/types.go`; a `store` string sent on the wire is refused at intake by the order-type allow-list.
...
| `no_payload` | No unclaimed payload of the requested type at the specified source node. |
...
| `missing_source` | A move or store order was submitted without a required source node. |
