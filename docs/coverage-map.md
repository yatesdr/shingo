...
- ~~`apiReplayOutbox` RequeueOutbox path in `handlers_diagnostics.go` — needs a real `*engine.ReconciliationService`; only input validation covered.~~ **CLOSED 2026-08-22.** The blocker was that `stubEngine.Reconciliation()` returned nil, so any test calling the handler panicked on its first method — the handler was not merely untested but untestable. The stub now returns a real service over `testDB` (`engine.NewReconciliationService`), and `handlers_replay_expiry_test.go` covers both the accept and the 409-on-expired paths.
