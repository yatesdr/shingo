...
- `MarkFaultedRecovered` is now the recovery path. It had no callers: the fleet reporting RUNNING mapped to `in_transit` and wrote "fleet reported in transit", the same row an order that was never in trouble writes — so 706 of 730 faults, the ones that recover, recorded nothing about themselves. The recovery row carries the dwell and the reason it recovered from.
- Grace expiry carries the fault's reason onto the terminal row, and `"grace_timeout"` becomes `protocol.TermGraceTimeout`. The poller has dropped its deadline entry by the time it fires, so the faulted history row is the last place that reason exists.
- `rds.fault_notice_after` (default 60s) is the one number that decides what an operator is told. Below it a faulted order is described as REPLANNING, at or above it as a FAULT with the fleet's reason. p50 recovery is 20s and p95 is 249s; 60s is three times the median and well inside any grace window. Config, not a constant, because it will be wrong at a plant whose replans run longer. Validated as `0 < notice < grace` but reported rather than fatal — a wording threshold must not stop a plant's core booting.
- One formatter, `protocol.FormatFaultSentence`, and one line builder, `protocol.BuildFaultLine`, feed Core's board, the order modal, the robots tiles and the Edge board. The threshold and both wordings are server-rendered; the browser ticks the durations and swaps the word when the clock crosses. `shared/utils.js` gains `installLiveDurations`, whose interval runs only while there are nodes to tick.
- No live order is ever described as failing. 706 of 730 faults recover on their own, and a floor told "failing" 730 times has learned to ignore the word by the time it matters. A table test walks every phase × ref × elapsed × notice combination, including a fleet that hands us the string "Task Failed" itself.
- The orders board stops reloading itself. `onSSE('order-update')` called `location.reload()` two seconds after any event, discarding the text filter and the scroll position each time — and no page that reloads every two seconds can host a one-second clock. Rows now come from a partial both the page and a fragment endpoint render, reconciled by order id, with a 30s sweep as the backstop for a dropped event.
- The board gains a Faulted status filter (there was none) and one summary chip counting only faults past the threshold. The robots tiles name the order each robot is on and its fault state. `/status` and the health strip gain a faulted gauge read from the DB, not from `Poller.FaultedCount()` — that is poller memory and reads zero after a restart.
- `/missions` gains a Faults card: outcomes with dwell percentiles, faults per day split replanning vs fault, and top-10 by robot, node and fleet reason. It uses a `LEAD` window rather than `DwellStats`, whose `MAX(from)→MAX(to)` pairing would put an order that faulted twice into two outcome buckets at once.
- Three additive `omitempty` fault fields plus the reference and the threshold cross to the Edge on `OrderUpdate`, `OrderStatusSnapshot` and `OrderProjection`; the Edge stores them (v36) and renders the same sentence with its own clock. Cleared from the status, never from a pushed empty value — an absent field and a deliberately empty one are identical on the wire, which is the queue_reason defect of 2026-08-03 pointed the other way.
- Two deliberate non-changes, named so nobody re-opens them by accident. There is no `FaultCode` vocabulary: 22 events under one vendor code is not a population to classify, `order_history.code` stays empty on faulted rows, and the vendor code is a reference. And no robot alarms appear on any fault surface — reflector and localization alarms fire constantly, and an unproven alarm printed beside a fault reads as its cause. The existing Failure Pareto, which does use robot alarms, is untouched.
- **Stranded in-transit bins are placed, not searched for.** A bin the robot had picked up sat at `_TRANSIT` with its claim released and nobody knowing where it physically was; the only exit was an operator finding it and pressing "I found it, it's at X". Core now asks the robot: an empty deck at a station that resolves places the bin there as `system:inferred`, a loaded deck moves it to a per-robot `_ROBOT:<vehicle>` synthetic node until the jack unloads, and anything else is an anomaly carrying the robot's last position (`bins.anomaly_note`, v96) so the search starts at a map pin.
- The placement goes through the SAME `RecoverTransitAnomaly` the operator's button calls, so the empty-node guard is never bypassed — an occupied node means the inference is stale and it falls through to the anomaly. A deck mid-travel is not an answer and does not get one.
- `jack_state`, `jack_isFull` and robot-level `isLoaded` are ingested; all three left the wire-drift allow-list, where they had been recorded as having no consumer. `jack_height` stays as the fallback proxy for a fleet that reports no state. There is no jack-unload EVENT — the jack is sampled state — so the unload is watched on the 2-second robot poll Core already makes, at no new RDS traffic.
- The RDS point → Core node mapping is IDENTITY, not a table: Core sends the node's own dot-name as the block Location, so the station names RDS reports back are Core node names. `CurrentStation` is empty while a robot is between points, so `LastStation` is the fallback.
- `ReconciliationService.Loop` re-runs the whole decision over every stranded bin — the guarantee behind the terminal-event fast path. **It is a backstop, not a backfill: it does not clear the pre-existing backlog, by design.** The inference reads the robot's CURRENT telemetry, which answers "where did this bin go" only while the robot has not moved on, so it considers `_TRANSIT` bins whose order ended inside `rds.stranded_sweep_window` (default 2h, comfortably longer than `fault_grace`) and leaves older ones as anomalies for an operator, exactly as before. An earlier cut swept everything regardless of age and described it as a feature; placing a days-old bin under whichever robot last held it invents a bin at a node the floor is then dispatched to fetch. Carrier (`_ROBOT:*`) bins are exempt from the window — the jack is a fact about now. The bins page keeps cargo and route decoration on a stranded bin (it lost them at exactly the moment the claim was released, so the bin most needing identification was the one the page said least about) and shows the inferred position; `/api/dispatch/anomalies` carries it too.
- Branch A places only from a PARKED robot. `ResolveRobotStation` falls back to `LastStation` when `CurrentStation` is empty, and `CurrentStation` is empty for most of the time a robot is moving — so without a `Busy` check the inference would place a bin at a node the robot merely drove past. The jack being at rest says nothing about the vehicle under it.
- A bin riding a robot is not a lost bin, and `/api/dispatch/anomalies` no longer says it is. `_ROBOT:*` nodes are synthetic and their bins unclaimed, which is the literal definition the anomaly query matched, so bins whose location was known exactly were listed as needing physical recovery — and the recovery button was live on them, which would have recorded a bin at a node while the real one was still on a moving deck. `RecoverTransitAnomaly` now refuses a carrier-node source as well as a synthetic destination. Excluded by prefix, not by narrowing back to `_TRANSIT`: a bin stray on a node group or lane root is still an anomaly nobody else lists. Carrier nodes are also deleted once nothing is on them, so they stop accumulating one permanent row per vehicle.
- The verdict goes degraded past HALF THE GRACE WINDOW, not past the notice threshold. The gauge keeps both counts, but 60s still fires on ordinary slow replans several times a shift, and a strip that cries wolf is not read. Past half of `fault_grace` an order is likelier to be written off than to recover, which is a degraded core and is rare enough to mean something.
- No new idle cost on the robot poll. The carried-bin watch moved behind the fleet-hash short-circuit (the jack is part of the hashed status, so an unload moves the hash by construction — the tick that matters is never the one skipped), the robots page's order lines are not built when no SSE client is connected, and the health gauge's per-faulted-order history read became one batched query. All three previously ran every 2 seconds, or per faulted order, whether or not anyone was watching.
- One duration ladder. `shared/utils.js formatDuration` now matches `protocol.FormatDuration` byte for byte, and the table is pinned in both suites. They had drifted on more than the space the original commit admitted — the JS kept seconds to an hour where Go drops them at ten minutes, zero-padded nothing, had a millisecond tier and no days tier, and rendered zero as `-` against Go's `0 s` — so every faulted row visibly rewrote itself one second after painting, and again after each reconcile.
- `/orders/rows` answers a failed read with 500, not 200 and an empty `<tbody>`. The fragment replaces the board's rows, so a DB blip rendered as "nothing is happening" on the page whose job is to say what is; the client now keeps the last good rows and logs.
- **Known issue, not fixed here:** `orders.DwellStats` pairs an order's OUTERMOST spans (`MAX(from)` → `MAX(to)`), so the /missions Dwell card over-reports for any order that passes through a state twice — a fault, a recovery and a second fault are counted as one long span. The Faults card is unaffected: it is built on a `LEAD` window that pairs each row with the one that actually followed it.
- `BinService.MarkAnomaly` finally has a non-test caller. Its comment claimed the failure path called it; the failure path stamps `anomaly_at` inline in SQL instead, so the comment described a real effect through a call site that did not exist.
- Migration heads: Core v96, Edge v36.

## 2026-08-22 — Messaging durability: outbox mute, expiry by class, snapshot cleanup, ledger rename

- The outbox doorbell was also a retry-cadence change and nobody noticed: a drain retries EVERY pending row, so under a transport failure each enqueue spent one of the message's `MaxRetries`. A publish failure now stands the wake arm down until a ticker drain succeeds, restoring the `MaxRetries × interval` tolerance — ~50 s at Springfield's interval, against ~20 s observed and ~0.5 s at the settle window's limit.
- `inventory.bin_uop_delta` and `inventory.lineside_bucket_delta` carry no expiry. Every other data subject is a snapshot whose successor carries the same truth seconds later; these are increments, and a dropped one is a permanently wrong count. Before the edge was hardwired, ~17 deltas a day arrived past the 5-minute default and were discarded at Core's ingestor — averaging 142 minutes late, peaking at 23 hours — while the edge recorded every one as sent. Safe to arrive late because Core dedups on `SequenceID` and routes a superseded generation to the discrepancy audit.
- A zero TTL now means no `exp` on the wire rather than "expires now": `now.Add(0)` stamped `exp = now`, which reads as expired on the next tick. Both stamping sites go through one helper so they cannot drift.
- Core counts expired-envelope drops, names the SUBJECT in the log line, and puts a 5-minute windowed count in the `/status` verdict. This was the larger of the two silent loss channels and nothing observed it: Springfield discarded 797 envelopes on 08-20 and 544 on 08-21 against zero on the four preceding days, with no signal anywhere. The subject is not on `RawHeader`, so it costs a second decode — taken only on the drop branch.
- Edge `/status` gains `dead_letters`, `kafka_last_publish_ok` and `kafka_last_publish_at`. `outbox_depth` cannot be read alone: during an outage it rises while rows retry and then FALLS AS THEY DIE, so a depth returning to zero reads exactly like recovery. `kafka_connected` keeps its name and value but is documented as "a writer exists" — `Connect()` does no I/O, so it cannot go false; use `kafka_last_publish_ok` for reachability.
- Dead letters are kept seven days, delivered rows one. A shared 24-hour cutoff destroyed the evidence twice in one week — once making a written claim that Springfield had never retried a message, which had to be withdrawn. The purge is two statements in one transaction; the `OR` could use no index on either arm, but that was worth milliseconds and is not why it split.
- The dead-letter Replay button refuses an expired envelope and says when it expired. Two production deltas were replayed on 08-22: the rows got `sent_at`, the edge logged "published", the dead-letter count fell by two, and Core discarded both because the envelopes had lapsed 23 hours earlier. `exp` is deliberately NOT re-stamped on replay.
- A full snapshot supersedes its unsent predecessors in the edge outbox, in one transaction, on an allow-list that REFUSES the delta subjects. After an outage the edge published an hour of superseded lineside reports in a burst, most of which Core discarded on arrival anyway.
- `plant.claims` publishes on change and on registration, hourly as a safety net — was every 5 minutes, one message per process, ~65/hour of unchanged config and 66% of everything Core discarded for expiry. Registration covers Core restarting: Core asks an unknown edge to re-register and the ack triggers a full snapshot.
- `apiCreateStyle` and `apiUpdateStyle` never signalled a spec change, so a created style or a rename left Core's mirror wrong. Every other mutation already did; the 5-minute timer had been hiding the gap.
- Core's lineside-report upsert is latest-wins on `reported_at`. It overwrote unconditionally, so a replayed or out-of-order report could move a row BACKWARDS past the 3-minute staleness window and drop that node back onto the pure ledger — the "reads STOCKED while the line starves" case the feed exists to correct.
- `edge_lineside_reports` rows older than seven days are pruned on Core's existing daily retention tick. Latest-wins cannot clear a row nothing will ever update again: one decommissioned station had been logging a 570-hour-stale fallback every monitor cycle.
- The lineside report feed is documented as LIVE. It has decided replenishment by default since 2026-07-24 (`lineside_decision_mode=edge_reports`), while eight comments and the file name still called it a shadow — which cost a full investigation that concluded it was dead code. `threshold_monitor_shadow.go` → `threshold_monitor_lineside.go`. The v52 DB table comment still says shadow; deferred to its own migration.
- `bin_uop_audit` → `bin_uop_ledger` (v95, metadata-only). It holds every applied delta with before/after totals and eight files read it as one; "audit" reads as archival, which is why nothing looked at what querying it cost. `bin_uop_exception` keeps its name — it is the audit. Earlier migrations' postconditions resolve under either name, because they are re-verified on every boot and would otherwise all report as failing.
- Applied deltas record their wire epoch in the ledger row. It was written only when a delta was DROPPED, so a delta that arrived on a stale generation and applied left no trace of which generation it carried.
- The two `/audit/*` routes are removed — zero callers in any page or script — and the `actor` index proposal is closed with them: they were the only queries filtering the ledger on a column that is 99.2% one value on an insert-only table.
- Two edge handlers were untestable rather than merely untested: `apiStatus` answered 503 because the test stub failed a type assertion, and `apiReplayOutbox` panicked because the stub's reconciliation service was nil. Both are now covered.
- Deferred, so nobody goes looking: `MaxRetries` is still a COUNT, so its real-world tolerance silently rescales with the drain cadence — expressing it as a duration is the open item. The active-dial half of the Kafka liveness probe is unbuilt. The active-order partial index and the edge single-connection instrumentation exist as design documents, not code.
- Migration heads: Core v95, Edge v35 (unchanged).
