...
- `ReconciliationService.Loop` re-runs the whole decision over every stranded bin — the guarantee behind the terminal-event fast path, and the thing that clears the backlog of bins already stranded before this shipped. The bins page keeps cargo and route decoration on a stranded bin (it lost them at exactly the moment the claim was released, so the bin most needing identification was the one the page said least about) and shows the inferred position; `/api/dispatch/anomalies` carries it too.
