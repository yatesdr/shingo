...
type SimConfig struct {
	...
	MaxSpeed   float64            `yaml:"max_speed"`   // effective-speed cap; <=0 → default (5×, the MEASURED ceiling — see clock.DefaultSimMaxSpeed). Past this the clock outruns the real choreography and the loop wedges, so requests are clamped. Must match core. Set very high to effectively uncap.
