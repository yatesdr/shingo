...
-- production_tick_cursor: the id of the last counter_snapshots row the
-- production tick shipper published. ONE row. Written at most once a minute and
-- only when it moved, plus once at a clean shutdown; after a crash the shipper
-- re-sends up to a minute of ticks, which Core's
-- (cell_id, edge_snapshot_id, recorded_at) key makes no-ops. A restore rewinds
-- it together with the ids it points into.
CREATE TABLE IF NOT EXISTS production_tick_cursor (
