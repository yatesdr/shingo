...
func (e *Engine) handleConsumeTick(node *processes.Node, runtime *processes.RuntimeState, claim *processes.NodeClaim, delta int) {
	...
	// the bin counter.
	remainder := drainLinesideFirst(e.db, node.ID, claim, delta)
...
func drainLinesideFirst(db *store.DB, nodeID int64, claim *processes.NodeClaim, delta int) int {
...
// ── Auto-release on staged (Bug 2 timing-window fix) ────────────────
