...
// pass that wrote none (no change, a reset to zero). The ring is a
// non-blocking signal; the poll never waits on the publish.
func TestProductionTick_NotifiesOncePerShippablePass(t *testing.T) {
	...
	r.pass(3) // no change
	r.pass(0) // reset to zero: delta 0, not shippable
	if got := rings.Load(); got != 1 {
