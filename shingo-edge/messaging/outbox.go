...
// the implicit dead-letter state (retries = MaxOutboxRetries).
func (s *edgeOutboxStore) MarkOutboxExhausted(id int64, reason string) error {
	return s.db.MarkOutboxExhausted(id, reason)
}

// PurgeOldOutbox removes delivered messages past the delivered cutoff and
// dead-lettered ones past their own, longer cutoff.
func (s *edgeOutboxStore) PurgeOldOutbox(delivered, deadLetter time.Duration) (int, error) {
	n, err := s.db.PurgeOldOutbox(delivered, deadLetter)
	return int(n), err
}
