...
// TestInventoryDeltaReporter_PendingBinIDsLifecycle pins the Item 2
// pending-tracking lifecycle: RecordBin marks the bin as pending,
// Flush clears it after a successful EnqueueOutbox, and a subsequent
// RecordBin re-marks it. The reconciler queries IsPendingBinDelta to
// decide whether to skip a heal — without correct lifecycle, it would
// either heal during in-flight deltas (correctness bug) or never heal
// (liveness bug).
func TestInventoryDeltaReporter_PendingBinIDsLifecycle(t *testing.T) {
	db := newReporterTestDB(t)
	r := NewInventoryDeltaReporter(db, "ALN_001")

	// Before any record: not pending.
	if r.IsPendingBinDelta(101) {
		t.Errorf("IsPendingBinDelta(101) before record = true, want false")
	}

	r.RecordBin(101, "PART-PEND", -5, protocol.ReasonConsumeTick)

	// After RecordBin: pending.
	if !r.IsPendingBinDelta(101) {
		t.Errorf("IsPendingBinDelta(101) after record = false, want true")
	}

	r.Flush()

	// After successful Flush: cleared.
	if r.IsPendingBinDelta(101) {
		t.Errorf("IsPendingBinDelta(101) after flush = true, want false (delta enqueued)")
	}

	// Re-record: pending again.
	r.RecordBin(101, "PART-PEND", -2, protocol.ReasonConsumeTick)
	if !r.IsPendingBinDelta(101) {
		t.Errorf("IsPendingBinDelta(101) after re-record = false, want true")
	}

	// Bucket symmetric: not-pending → record → pending → flush → not-pending.
	if r.IsPendingBucketDelta(7, 200, "PART-B") {
		t.Errorf("IsPendingBucketDelta(7,200,PART-B) before record = true, want false")
	}
	r.RecordBucket(7, "L1|U1", 200, "PART-B", -3, protocol.ReasonConsumeDrain)
	if !r.IsPendingBucketDelta(7, 200, "PART-B") {
		t.Errorf("IsPendingBucketDelta(7,200,PART-B) after record = false, want true")
	}
	r.Flush()
	if r.IsPendingBucketDelta(7, 200, "PART-B") {
		t.Errorf("IsPendingBucketDelta(7,200,PART-B) after flush = true, want false")
	}
}

// TestPendingBinIDs_RestoredFromOutboxAtStartup pins the post-crash
// recovery path: deltas already enqueued in the outbox at startup
// must populate the pending sets via LoadPendingFromOutbox. Without
// this, a reporter constructed against an outbox carrying unsent
// envelopes would report nothing as pending, and the reconciler's
// first pass after restart would heal scopes whose in-flight delta
// hasn't reached Core yet — corrupting the local cache the same way
// pre-Item-2 behavior did.
func TestPendingBinIDs_RestoredFromOutboxAtStartup(t *testing.T) {
	db := newReporterTestDB(t)

	// Phase 1: instance "before crash" enqueues two bin deltas and
	// one bucket delta, but does NOT mark them sent.
	r1 := NewInventoryDeltaReporter(db, "ALN_001")
	r1.RecordBin(501, "PART-X", -7, protocol.ReasonConsumeTick)
	r1.RecordBin(502, "PART-Y", -3, protocol.ReasonConsumeTick)
	r1.RecordBucket(11, "L1|U1", 300, "PART-Z", -4, protocol.ReasonConsumeDrain)
	r1.Flush()

	// Sanity: the outbox holds three unsent envelopes.
	binDeltas := pendingOutboxByType[protocol.BinUOPDelta](t, db, protocol.SubjectBinUOPDelta)
	bucketDeltas := pendingOutboxByType[protocol.LinesideBucketDelta](t, db, protocol.SubjectLinesideBucketDelta)
	if len(binDeltas) != 2 || len(bucketDeltas) != 1 {
		t.Fatalf("outbox shape: %d bin / %d bucket envelopes, want 2 / 1",
			len(binDeltas), len(bucketDeltas))
	}

	// Phase 2: simulate process restart — fresh reporter, same db.
	r2 := NewInventoryDeltaReporter(db, "ALN_001")

	// Pre-load: nothing should be pending yet (in-memory map is empty).
	if r2.IsPendingBinDelta(501) || r2.IsPendingBinDelta(502) {
		t.Errorf("fresh reporter reports bin pending without LoadPendingFromOutbox call")
	}

	if err := r2.LoadPendingFromOutbox(); err != nil {
		t.Fatalf("LoadPendingFromOutbox: %v", err)
	}

	// Post-load: both bins and the bucket scope must be pending.
	if !r2.IsPendingBinDelta(501) {
		t.Errorf("IsPendingBinDelta(501) after restore = false, want true")
	}
	if !r2.IsPendingBinDelta(502) {
		t.Errorf("IsPendingBinDelta(502) after restore = false, want true")
	}
	if !r2.IsPendingBucketDelta(11, 300, "PART-Z") {
		t.Errorf("IsPendingBucketDelta after restore = false, want true")
	}
	// A bin we never recorded must remain not-pending.
	if r2.IsPendingBinDelta(999) {
		t.Errorf("IsPendingBinDelta(999) after restore = true, want false (never recorded)")
	}
}
