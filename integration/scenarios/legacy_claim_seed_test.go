...
// CycleMode descriptor and as legacy DB rows. Several changeover/release
// scenarios use "simple" as a cheap base mode and rely on the changeover
// planner's bare-move default, so those seeds must still produce a
// swap_mode='simple' row.
//
...
// blank mode. Scenario claim seeds route through this shim instead of calling
// UpsertStyleNodeClaim directly (a mechanical sed), so a single seam owns the
