...
func (d *Dispatcher) resolveSyntheticDropoff(order *orders.Order) error {
	...
	result, rErr := d.resolver.Resolve(node, binresolver.ResolveModeStore, order.PayloadCode,
		binresolver.UnknownBinType("scanner deferral keeper: order holds no bin and no hold"), digAskerFor(order), nil)
...
func (d *Dispatcher) redirectStoreOffDugLane(order *orders.Order) {
	...
	result, err := d.resolver.Resolve(group, binresolver.ResolveModeStore, order.PayloadCode,
		binresolver.UnknownBinType("dug-lane redirect: order holds no bin and no hold"), digAskerFor(order), nil)
