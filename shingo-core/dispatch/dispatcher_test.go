...
func dispatchSimpleViaScanner(t *testing.T, d *Dispatcher, db *store.DB, orderUUID string) *orders.Order {
	...
	if dest := d.ReserveStorageDropoff(o); dest.Refused() {
		t.Fatalf("scanner-mirror reserve dropoff %s: %v (%s)", orderUUID, dest.Err, dest.Cause)
	}
...
func TestHandleOrderRequest_Move_NoPayloadAtPickup(t *testing.T) {
	...
	// Behavior change: a move whose concrete source node has no
	...
	// move-no-source fix direction and the operator-driven give-up rule (a move
	// sources the same node on replay; the bin may simply not have arrived yet).
	// Was: failed=1, code=no_payload.
