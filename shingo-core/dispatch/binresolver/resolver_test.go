...
func TestDefaultResolver_Retrieve_PicksFirstChildWithAvailableBin(t *testing.T) {
	...
	got, err := r.Resolve(parent, "retrieve", "P1", NoBinType, reservations.Anyone, nil)
	if err != nil {
...
func TestDefaultResolver_Retrieve_NoAvailableBins(t *testing.T) {
	...
	r := &DefaultResolver{DB: f}
	if _, err := r.Resolve(parent, "retrieve", "P1", NoBinType, reservations.Anyone, nil); err == nil {
...
func TestDefaultResolver_Store_PicksConsolidationCandidate(t *testing.T) {
	...
	r := &DefaultResolver{DB: f}
	got, err := r.Resolve(parent, "store", "P1", UnknownBinType("test: caller names no carrier"), reservations.Anyone, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got.Node != b {
...
func TestDefaultResolver_Store_NoCandidate(t *testing.T) {
	...
	r := &DefaultResolver{DB: f}
	if _, err := r.Resolve(parent, "store", "P1", UnknownBinType("test: caller names no carrier"), reservations.Anyone, nil); err == nil {
...
func TestDefaultResolver_UnknownOrderType_FirstEnabled(t *testing.T) {
	...
	r := &DefaultResolver{DB: f}
	got, err := r.Resolve(parent, "weird", "", NoBinType, reservations.Anyone, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got.Node != on {
...
func TestDefaultResolver_NoChildren(t *testing.T) {
	...
	r := &DefaultResolver{DB: f}
	if _, err := r.Resolve(parent, "retrieve", "", NoBinType, reservations.Anyone, nil); err == nil {
...
func TestDefaultResolver_Retrieve_NGRPDelegatesToGroupResolver(t *testing.T) {
	...
	r := &DefaultResolver{DB: f}
	got, err := r.Resolve(ngrp, "retrieve", "P1", NoBinType, reservations.Anyone, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
