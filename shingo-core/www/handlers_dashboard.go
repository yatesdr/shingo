...
func (h *Handlers) handleDashboard(w http.ResponseWriter, r *http.Request) {
	h.renderHome(w, r, "dashboard.html")
}

func (h *Handlers) renderHome(w http.ResponseWriter, r *http.Request, tmpl string) {
	...
	if len(activeOrders) > 5 {
		data["RecentOrders"] = activeOrders[:5]
	} else {
		data["RecentOrders"] = activeOrders
	}

	h.render(w, r, tmpl, data)
}
