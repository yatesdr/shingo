...
func (h *Handlers) handleMissionDetail(w http.ResponseWriter, r *http.Request) {
	...
	order, err := h.engine.DB().GetOrder(orderID)
	if err != nil {
...
func (h *Handlers) apiListMissions(w http.ResponseWriter, r *http.Request) {
	...
	missions, total, err := h.engine.DB().ListMissions(f)
	if err != nil {
...
func (h *Handlers) apiGetMission(w http.ResponseWriter, r *http.Request) {
	...
	telemetry, _ := h.engine.DB().GetMissionTelemetry(orderID)
	events, _ := h.engine.DB().ListMissionEvents(orderID)
	history, _ := h.engine.DB().ListOrderHistory(orderID)
...
func (h *Handlers) apiMissionStats(w http.ResponseWriter, r *http.Request) {
	f := parseMissionFilter(r)
	stats, err := h.engine.DB().GetMissionStats(f)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
