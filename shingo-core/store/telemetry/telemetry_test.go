...
func TestCoverage_InsertAndListMissionEvents(t *testing.T) {
	...
	if err := telemetry.InsertEvent(db.DB, e1); err != nil {
	...
	if err := telemetry.InsertEvent(db.DB, e2); err != nil {
...
func TestCoverage_UpsertAndGetMissionTelemetry(t *testing.T) {
	...
	if err := telemetry.UpsertMission(db.DB, mt); err != nil {
		t.Fatalf("UpsertMission (insert): %v", err)
	}
	...
	mt.RobotID = "AMB-2"
	if err := telemetry.UpsertMission(db.DB, mt); err != nil {
		t.Fatalf("UpsertMission (update): %v", err)
	}
...
func TestCoverage_GetMissionStats(t *testing.T) {
	...
	for _, r := range rows {
		telemetry.UpsertMission(db.DB, r)
	}
