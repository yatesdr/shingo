...
func AssignStation(db *sql.DB, nodeID int64, stationID string) error {
	_, err := db.Exec(`INSERT INTO node_stations (node_id, station_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, nodeID, stationID)
	return err
}

// UnassignStation removes a station ID from a node.
func UnassignStation(db *sql.DB, nodeID int64, stationID string) error {
	_, err := db.Exec(`DELETE FROM node_stations WHERE node_id=$1 AND station_id=$2`, nodeID, stationID)
	return err
}

// ListStationsForNode returns the directly-assigned stations for a node.
func ListStationsForNode(db *sql.DB, nodeID int64) ([]string, error) {
...
func ListNodesForStation(db *sql.DB, stationID string) ([]*Node, error) {
...
//   - "none": empty set (no stations permitted)
//   - "specific": returns directly assigned stations
//   - "" / "inherit": walks parent chain until a non-empty set is found
func GetEffectiveStations(db *sql.DB, nodeID int64) ([]string, error) {
	mode := GetProperty(db, nodeID, "station_mode")
	switch mode {
	case "all":
		return nil, nil
	case "none":
...
// SetStations replaces all station assignments for a node.
func SetStations(db *sql.DB, nodeID int64, stationIDs []string) error {
	tx, err := db.Begin()
	if err != nil {
		return err
	}
