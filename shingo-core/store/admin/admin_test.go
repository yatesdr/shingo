...
func TestCoverage_AdminUserExists_Empty(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("AnyExists empty: %v", err)
	}
	if exists {
		t.Error("AnyExists on empty table should be false")
	}
}
...
func TestCoverage_CreateAdminUserAndGet(t *testing.T) {
	t.Parallel()
	db := testdb.Open(t)
	if err := admin.Create(db.DB, "alice", "hash-alice"); err != nil {
	...
	exists, err := admin.AnyExists(db.DB)
	if err != nil {
		t.Fatalf("AnyExists after create: %v", err)
	}
	if !exists {
		t.Error("AnyExists should be true after insert")
	}
}
...
func TestCoverage_GetAdminUser_Miss(t *testing.T) {
	...
	if err == nil {
		t.Error("Get miss should return error, got nil")
	}
}
...
func TestCoverage_CreateAdminUser_MultipleUsers(t *testing.T) {
	t.Parallel()
	db := testdb.Open(t)
	if err := admin.Create(db.DB, "alice", "h1"); err != nil {
		t.Fatalf("create alice: %v", err)
	}
	if err := admin.Create(db.DB, "bob", "h2"); err != nil {
