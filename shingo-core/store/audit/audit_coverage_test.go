...
func TestCoverage_AuditLog(t *testing.T) {
	...
	if len(entries) != 3 { t.Errorf("len = %d, want 3", len(entries)) }
	if entries[0].Action != "updated" { t.Errorf("first entry action = %q, want %q", entries[0].Action, "updated") }
	orderEntries, _ := audit.ListForEntity(db.DB, "order", 1)
	if len(orderEntries) != 2 { t.Errorf("order entries = %d, want 2", len(orderEntries)) }
}
