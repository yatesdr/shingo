...
// restore-blockers table is gone (v70) and the head version is 70.
//
...
func TestMigrate_PendingRestocksRetired(t *testing.T) {
	...
	if got := store.LatestMigrationVersion(); got != 70 {
		t.Errorf("head migration = %d, want 70", got)
	}
}
