...
func TestCoverage_ScenePoint_UpsertInsertThenUpdate(t *testing.T) {
	...
	if err := scene.Upsert(db.DB, sp); err != nil {
	...
	if all[0].Label != "Home Pose" {
		t.Errorf("Label = %q, want Home Pose", all[0].Label)
	}
	sp.PosX = 100.0
	sp.PosY = 200.0
	sp.Label = "New Home"
	...
	if len(all2) != 1 {
		t.Fatalf("after upsert-update len = %d, want 1", len(all2))
	}
...
func TestCoverage_ScenePoint_ListFiltersAndDelete(t *testing.T) {
	...
	for _, p := range waypoints {
		if p.ClassName != "Waypoint" {
			t.Errorf("unexpected class %q", p.ClassName)
		}
	}
	areaA, err := scene.ListByArea(db.DB, "AREA-A")
	if err != nil {
		t.Fatalf("ListByArea AREA-A: %v", err)
	}
	if len(areaA) != 2 {
	...
	for _, p := range areaA {
		if p.AreaName != "AREA-A" {
			t.Errorf("unexpected area %q", p.AreaName)
		}
	}
	if err := scene.DeleteByArea(db.DB, "AREA-A"); err != nil {
