...
-- cell_part_events: one row per PLC counter tick, projected from production.ticks
-- (and production.tick from older Edges). Partitioned monthly by recorded_at;
-- the partition manager (store/heartbeat) creates the current+next month
-- partitions at boot and drops partitions older than the retention window
-- nightly. The dedup key is the unique index uq_cell_part_events_tick on
-- (cell_id, edge_snapshot_id, recorded_at), created by migration v128 and not
-- here: built in the baseline, it would run ahead of v128's duplicate collapse
-- on a plant database. cell_id is the envelope's station (the cell identity the
-- /api/cells endpoints key on).
